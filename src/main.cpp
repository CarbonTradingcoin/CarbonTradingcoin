...
uint256 hashGenesisBlock("0x3854d5273ef33ee3b88f8cff02c36b9a63bc9be12f280293c763f9f04451d3e3");
uint256 hashMerkleRootBlock("0xa4078373708a0fe10425b4c812dd95719d451be9a9a21bf764403422df46af85");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Carbonemissiontradecoin: starting difficulty is 1 / 2^12
...
int64 nChainStartTime = 1393891200; //Human time (GMT): Tue, 04 Mar 2014 00:00:00 GMT
...
const string strMessageMagic = "Carbonemissiontradecoin Signed Message:\n";
...
bool CTxOut::IsDust() const
{
    ...
    // so dust is a txout less than 54 uCT
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    if(nHeight>216000)
        nSubsidy >>= ((nHeight-216000) / 4204800); // CT: 840k blocks in ~4 years
    if(nSubsidy<1 * COIN) nSubsidy = 1 * COIN;

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 80 * 0.5 * 60; // Vertcoin: 80 blocks diff retarget
static const int64 nTargetSpacing = 0.5 * 60; // Vertcoin: 0.5 minutes
...
unsigned int static GetNextWorkRequired_V2(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    static const int64     BlocksTargetSpacing1           = 0.5 * 60; // 0.5 minutes
    unsigned int           TimeDaySeconds                = 60 * 60 * 24;
    int64                  PastSecondsMin                = TimeDaySeconds * 0.25;
    int64                  PastSecondsMax                = TimeDaySeconds * 7;
    uint64                 PastBlocksMin                 = PastSecondsMin / BlocksTargetSpacing;
    uint64                 PastBlocksMax                 = PastSecondsMax / BlocksTargetSpacing;
    uint64                 PastBlocksMin1                 = PastSecondsMin / BlocksTargetSpacing1;
    uint64                 PastBlocksMax1                 = PastSecondsMax / BlocksTargetSpacing1;

    if(pindexLast->nHeight+1<=36000)
        return KimotoGravityWell(pindexLast, pblock, BlocksTargetSpacing, PastBlocksMin, PastBlocksMax);
    else
        return KimotoGravityWell(pindexLast, pblock, BlocksTargetSpacing1, PastBlocksMin1, PastBlocksMax1);
}
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    if (fTestNet) {
        ...
        if (pindexLast->nHeight+1 >= 28800) { DiffMode = 2; }  //CT, 10 days after Release Time
    }
...
void ThreadScriptCheck() {
    RenameThread("carbonemissiontradecoin-scriptch");
...
bool CBlock::ConnectBlock(CValidationState &state, CBlockIndex* pindex, CCoinsViewCache &view, bool fJustCheck)
{
    ...
    printf("GetHash()=%s, hashGenesisBlock=%s\n",GetHash().ToString().c_str(),hashGenesisBlock.ToString().c_str());
...
bool VerifyDB(int nCheckLevel, int nCheckDepth){
    ...
    if(nCheckLevel==0) nCheckLevel = GetArg("-checklevel", 3);
    if(nCheckDepth==0) nCheckDepth = GetArg( "-checkblocks", 288);
    if (nCheckDepth <= 0)
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "CarbonEmissionTradeCoin - Trade Carbon with this coin!";
        ...
        block.nTime    = nChainStartTime;//Human time (GMT): Tue, 04 Mar 2014 00:00:00 GMT
        block.nNonce   = 596782;
        ...
        printf("hash.ToString().c_str()=%s\n", hash.ToString().c_str());
        printf("hashGenesisBlock.ToString().c_str()=%s\n", hashGenesisBlock.ToString().c_str());
        printf("block.hashMerkleRoot.ToString().c_str()=%s\n", block.hashMerkleRoot.ToString().c_str());
        assert(block.hashMerkleRoot == hashMerkleRootBlock);
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc1, 0xb6, 0xdb };
            ...
            for (unsigned int i = 0; i < vWorkQueue.size(); i++)
            {
                     ...
                     ++mi)
                {
                    ...
                    if (tx.AcceptToMemoryPool(stateDummy, true, true, &fMissingInputs2))
                    {
                        ...
                        RelayTransaction(orphanTx, orphanHash,vMsg);
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    //// debug print
    printf("CarbonemissiontradecoinMiner:\n");
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("CarbonemissiontradecoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("CarbonemissiontradecoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static CarbonemissiontradecoinMiner(CWallet *pwallet)
{
    printf("CarbonemissiontradecoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("carbonemissiontradecoin-miner");
    ...
    try { loop {
            ...
            printf("Running CarbonemissiontradecoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("CarbonemissiontradecoinMiner terminated\n");
        throw;
    }
}

void GenerateCarbonemissiontradecoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&CarbonemissiontradecoinMiner, pwallet));
}
