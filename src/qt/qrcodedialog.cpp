...
{
    QString ret = QString("carbonemissiontradecoin:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non CT unit input in lnReqAmount, we generate the URI with CT as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(CarbonemissiontradecoinUnits::format(CarbonemissiontradecoinUnits::CT, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
