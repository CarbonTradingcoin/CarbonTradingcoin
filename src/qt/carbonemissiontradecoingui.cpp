/*
 * Qt4 carbonemissiontradecoin GUI.
 *
 * W.J. van der Laan 2011-2012
 * The Carbonemissiontradecoin Developers 2011-2012
 */

#include "carbonemissiontradecoingui.h"
...
const QString CarbonemissiontradecoinGUI::DEFAULT_WALLET = "~Default";

CarbonemissiontradecoinGUI::CarbonemissiontradecoinGUI(bool fIsTestnet, QWidget *parent) :
    QMainWindow(parent),
    ...
    prevBlocks(0)
{
    ...
    if (!fIsTestnet)
    {
        setWindowTitle(tr("Carbonemissiontradecoin") + " - " + tr("Wallet"));
        QApplication::setWindowIcon(QIcon(":icons/carbonemissiontradecoin"));
        setWindowIcon(QIcon(":icons/carbonemissiontradecoin"));
    }
    else
    {
        setWindowTitle(tr("Carbonemissiontradecoin") + " - " + tr("Wallet") + " " + tr("[testnet]"));
        QApplication::setWindowIcon(QIcon(":icons/carbonemissiontradecoin_testnet"));
        setWindowIcon(QIcon(":icons/carbonemissiontradecoin_testnet"));
    }
#else
    ...
    if (!fIsTestnet)
        MacDockIconHandler::instance()->setIcon(QIcon(":icons/carbonemissiontradecoin"));
    else
        MacDockIconHandler::instance()->setIcon(QIcon(":icons/carbonemissiontradecoin_testnet"));
#endif
...
void CarbonemissiontradecoinGUI::RunMiningAsStartup()
{
    OptionsModel * opModel = clientModel->getOptionsModel();
    bool fGenerate = opModel->getStartMiningAtStartup();
    mapArgs["-gen"] = (fGenerate ? "1" : "0");
    GenerateCarbonemissiontradecoins(fGenerate, pwalletMain);
    ui->checkBox->setChecked(fGenerate);
    miningPage->SetMiningStatus(GetBoolArg("-gen", false));
    return;
}

void CarbonemissiontradecoinGUI::SetMiningStatus(bool isMining)
{
    if(ui->checkBox)
        ui->checkBox->setChecked(isMining);
}

CarbonemissiontradecoinGUI::~CarbonemissiontradecoinGUI()
{
    //saveWindowGeometry();
...
void CarbonemissiontradecoinGUI::createActions(bool fIsTestnet)
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a Carbonemissiontradecoin address"));
    ...
    if (!fIsTestnet)
        aboutAction = new QAction(QIcon(":/icons/carbonemissiontradecoin"), tr("&About Carbonemissiontradecoin"), this);
    else
        aboutAction = new QAction(QIcon(":/icons/carbonemissiontradecoin_testnet"), tr("&About Carbonemissiontradecoin"), this);
    aboutAction->setStatusTip(tr("Show information about Carbonemissiontradecoin"));
    aboutAction->setMenuRole(QAction::AboutRole);
    aboutQtAction = new QAction(QIcon(":/trolltech/qmessagebox/images/qtlogo-64.png"), tr("About &Qt"), this);
    aboutQtAction->setStatusTip(tr("Show information about Qt"));
    aboutQtAction->setMenuRole(QAction::AboutQtRole);
    optionsAction = new QAction(QIcon(":/icons/options"), tr("&Options..."), this);
    optionsAction->setStatusTip(tr("Modify configuration options for Carbonemissiontradecoin"));
    optionsAction->setMenuRole(QAction::PreferencesRole);
    if (!fIsTestnet)
        toggleHideAction = new QAction(QIcon(":/icons/carbonemissiontradecoin"), tr("&Show / Hide"), this);
    else
        toggleHideAction = new QAction(QIcon(":/icons/carbonemissiontradecoin_testnet"), tr("&Show / Hide"), this);
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your Carbonemissiontradecoin addresses to prove you own them"));
    verifyMessageAction = new QAction(QIcon(":/icons/transaction_0"), tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified Carbonemissiontradecoin addresses"));
...
void CarbonemissiontradecoinGUI::createMenuBar()
{
#ifdef Q_OS_MAC
...
void CarbonemissiontradecoinGUI::createToolBars()
{
...
void CarbonemissiontradecoinGUI::setClientModel(ClientModel *clientModel)
{
...
void CarbonemissiontradecoinGUI::setWalletModel(WalletModel *walletModel)
{
...
void CarbonemissiontradecoinGUI::createTrayIcon()
{
#ifndef Q_OS_MAC
    trayIcon = new QSystemTrayIcon(this);

    trayIcon->setToolTip(tr("Carbonemissiontradecoin client"));
...
void CarbonemissiontradecoinGUI::createTrayIconMenu()
{
    QMenu *trayIconMenu;
#ifndef Q_OS_MAC
...
#ifndef Q_OS_MAC
void CarbonemissiontradecoinGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
void CarbonemissiontradecoinGUI::saveWindowGeometry()
{
    QSettings settings;
...
void CarbonemissiontradecoinGUI::restoreWindowGeometry()
{
    QSettings settings;
...
void CarbonemissiontradecoinGUI::optionsClicked()
{
...
void CarbonemissiontradecoinGUI::aboutClicked()
{
...
void CarbonemissiontradecoinGUI::setNumConnections(int count)
{
    QString icon;
    switch(count)
    {
    ...
    labelConnectionsIcon->setToolTip(tr("%n active connection(s) to Carbonemissiontradecoin network", "", count));
}

void CarbonemissiontradecoinGUI::setNumBlocks(int count, int nTotalBlocks)
{
...
void CarbonemissiontradecoinGUI::message(const QString &title, const QString &message, unsigned int style, bool *ret)
{
    QString strTitle = tr("Carbonemissiontradecoin"); // default title
...
void CarbonemissiontradecoinGUI::changeEvent(QEvent *e)
{
...
void CarbonemissiontradecoinGUI::closeEvent(QCloseEvent *event)
{
    if(clientModel)
    {
#ifndef Q_OS_MAC // Ignored on Mac
...
void CarbonemissiontradecoinGUI::askFee(qint64 nFeeRequired, bool *payFee)
{
    QString strMessage = tr("This transaction is over the size limit. You can still send it for a fee of %1, "
        "which goes to the nodes that process your transaction and helps to support the network. "
        "Do you want to pay the fee?").arg(CarbonemissiontradecoinUnits::formatWithUnit(CarbonemissiontradecoinUnits::CT, nFeeRequired));
...
//void CarbonemissiontradecoinGUI::incomingTransaction(const QString& date, int unit, qint64 amount, const QString& type, const QString& address)
void CarbonemissiontradecoinGUI::incomingTransaction(const QModelIndex& parent, int start, int /*end*/)
{
    ...
    message((amount)<0 ? tr("Sent transaction") : tr("Incoming transaction"),
             tr("Date: %1\n"
                ...
                "Address: %4\n")
                  .arg(date)
                  .arg(CarbonemissiontradecoinUnits::formatWithUnit(walletModel->getOptionsModel()->getDisplayUnit(), amount, true))
                  .arg(type)
                  .arg(address), CClientUIInterface::MSG_INFORMATION);
}

void CarbonemissiontradecoinGUI::gotoOverviewPage()
{
...
void CarbonemissiontradecoinGUI::gotoHistoryPage()
{
...
void CarbonemissiontradecoinGUI::gotoAddressBookPage()
{
...
void CarbonemissiontradecoinGUI::gotoReceiveCoinsPage()
{
...
//void CarbonemissiontradecoinGUI::gotoSendCoinsPage(QString addr)
void CarbonemissiontradecoinGUI::gotoSendCoinsPage()
{
...
void CarbonemissiontradecoinGUI::gotoSignMessageTab(QString addr)
{
...
void CarbonemissiontradecoinGUI::gotoVerifyMessageTab(QString addr)
{
...
void CarbonemissiontradecoinGUI::gotoServiceMessagesPage()
{
...
void CarbonemissiontradecoinGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void CarbonemissiontradecoinGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
	    ...
	    gotoSendCoinsPage();
            ...
            message(tr("URI handling"), tr("URI can not be parsed! This can be caused by an invalid Carbonemissiontradecoin address or malformed URI parameters."),
...
bool CarbonemissiontradecoinGUI::eventFilter(QObject *object, QEvent *event)
{
...
void CarbonemissiontradecoinGUI::resizeEvent(QResizeEvent *e)
{
    updateMask();
    QMainWindow::resizeEvent(e);
}

void CarbonemissiontradecoinGUI::paintEvent(QPaintEvent *e)
{
    Q_UNUSED(e);
    ...
    painter.setBrush(QBrush(QColor(70,120,16)));

    QMainWindow::paintEvent(e);
}

void CarbonemissiontradecoinGUI::updateMask()
{
    // main form mask
    _mask = QBitmap(size());
    _mask.clear();
...
void CarbonemissiontradecoinGUI::handleURI(QString strURI)
{
    // URI has to be valid
...
void CarbonemissiontradecoinGUI::menuFileRequested()
{
    QMenu menu(this);
...
void CarbonemissiontradecoinGUI::menuOperationsRequested()
{
    QMenu menu(this);
    QAction* send = menu.addAction(tr("Send Carbonemissiontradecoins"));
    QAction* receive = menu.addAction(tr("Receive Carbonemissiontradecoins"));
...
void CarbonemissiontradecoinGUI::menuSettingsRequested()
{
    QMenu menu(this);
    QAction* commonAndNetwork = menu.addAction(tr("Common, Network"));
    QAction* debugWindow = menu.addAction(tr("&Debug window").remove('&'));
    QAction* about = menu.addAction(tr("&About Carbonemissiontradecoin").remove('&'));    
...
void CarbonemissiontradecoinGUI::setEncryptionStatus(int status)
{
    switch(status)
    {
...
void CarbonemissiontradecoinGUI::encryptWallet(bool status)
{
    if(!walletModel)
        return;
...
void CarbonemissiontradecoinGUI::backupWallet()
{
...
void CarbonemissiontradecoinGUI::changePassphrase()
{
...
void CarbonemissiontradecoinGUI::unlockWallet()
{
    if(!walletModel)
        return;
...
void CarbonemissiontradecoinGUI::showNormalIfMinimized(bool fToggleHidden)
{
...
void CarbonemissiontradecoinGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void CarbonemissiontradecoinGUI::detectShutdown()
{
    if (ShutdownRequested())
        QMetaObject::invokeMethod(QCoreApplication::instance(), "quit", Qt::QueuedConnection);
}

void CarbonemissiontradecoinGUI::onMiningClicked()
{
    //ui->wMining->setStyleSheet("background-color:black;");
    centralWidget->setCurrentWidget(miningPage);
}

void CarbonemissiontradecoinGUI::on_bHelp_clicked()
{    
    rpcConsole->show();
}
